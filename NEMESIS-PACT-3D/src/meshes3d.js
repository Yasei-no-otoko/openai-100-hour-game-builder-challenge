/* NEMESIS PACT 0.3.4 — procedural mesh scene; no simulation mutations.
 * 2D mechanics are unchanged. 3D scenery uses seeded pass-height layers, projected shadows,
 * stronger stage-specific architecture, and light ambient animation.
 */
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.PactMeshes=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){'use strict';
 ...
 class Scene{
  ...
  dynamicScenery(w,h,stage=0,seed='TITLE',t=0){
   ...
   for(let i=0;i<(stage===2?3:2);i++){
    const dir=i%2?-1:1,rr=innerR*(.84+.14*i),ang=t*(.10+.05*i)*dir+i*.7;
    this.add('ring',cx,cy,-18+i*4,rr,rr,2.5+i,ang,accent,.28,.45,.32+.12*i,LAYER.TRIM);
   }
   if(stage===0){
    for(let i=0;i<10;i++){
      const a=i*TAU/10+t*.08+(i%2)*.07,rr=innerR*.72,x=cx+Math.cos(a)*rr,y=cy+Math.sin(a)*rr;
      const pulse=.5+.5*Math.sin(t*1.2+i*.8),hz=42+26*pulse;
      this.box(x,y,4+hz*.22,10,10,46+hz,a,[.15,.24,.24],.25,.85,0,LAYER.HIGH);
      this.box(x,y,26+hz*.45,5,20,4,a+Math.PI/2,accent,.20,.18,1.8+.8*pulse,LAYER.MID);
    }
    for(let i=0;i<4;i++){
      const a=i*TAU/4+t*.04,len=innerR*.80;
      this.box(cx,cy,-8,len,5,5,a,[.12,.20,.21],.30,.65,0,LAYER.LOW);
      for(let j=0;j<4;j++){const f=((t*.22+j*.27+i*.12)%1),px=cx+Math.cos(a)*(f*len-len/2),py=cy+Math.sin(a)*(f*len-len/2);this.add('orb',px,py,-2,2.2,2.2,2.2,0,accent,.14,.08,2.4,LAYER.TRIM);} 
    }
   }else if(stage===1){
    for(let i=0;i<8;i++){
      const side=(i%4)-1.5,vertical=i<4,rise=.5+.5*Math.sin(t*1.6+i*.7),x=vertical?cx+side*innerR*.34:cx+side*innerR*.62,y=vertical?cy+side*innerR*.62:cy+side*innerR*.34;
      this.box(x,y,12+rise*18,8,8,42+28*rise,0,[.18,.16,.23],.22,.88,0,LAYER.HIGH);
      this.box(x,y,44+rise*26,4,18,4,0,accent,.22,.16,2.3,LAYER.TRIM);
    }
    for(let lane=0;lane<4;lane++){
      ...
      for(let j=0;j<5;j++){const f=((t*.36+j*.19+lane*.11)%1),px=(a===0?cx-len/2+f*len:cx+off),py=(a===0?cy+off:cy-len/2+f*len);this.add('orb',px,py,-5,2.0,2.0,2.0,0,accent,.12,.05,2.6,LAYER.TRIM);} 
    }
   }else{
    for(let i=0;i<12;i++){
      const a=i*TAU/12-t*.06,rr=innerR*.64,lift=.5+.5*Math.sin(t*1.1+i*.6),x=cx+Math.cos(a)*rr,y=cy+Math.sin(a)*rr;
      this.box(x,y,8+lift*16,11,11,34+26*lift,a,[.24,.20,.14],.22,.82,0,LAYER.MID);
      this.add('orb',x,y,38+lift*18,2.8,2.8,2.8,0,accent,.15,.07,2.8,LAYER.TRIM);
    }
    for(let loop=0;loop<2;loop++){
      const rr=innerR*(.50+.22*loop),count=10+loop*4;
      for(let i=0;i<count;i++){const a=i*TAU/count+t*(loop?-.18:.14),x=cx+Math.cos(a)*rr,y=cy+Math.sin(a)*rr;this.add('orb',x,y,-3,1.9+loop*.3,1.9+loop*.3,1.9+loop*.3,0,accent,.12,.04,2.5,LAYER.TRIM);} 
    }
   }
   if(theme===3){for(let i=0;i<6;i++){const a=i*TAU/6+t*.03,rr=ringR*(.88+.03*Math.sin(t+i));this.box(cx+Math.cos(a)*rr,cy+Math.sin(a)*rr,18,16,8,22,a,[base[0],base[1],base[2]],.25,.84,.3,LAYER.MID);}}
  }
  ...
  floor(w,h,stage=0,seed='TITLE'){
   ...
   for(const x of [edgeInset,w-edgeInset]){
    this.box(x,h/2,-26,16,h-20,74,0,[.11,.16,.19],.27,.88,0,TALL);
    this.box(x+(x<w/2?9:-9),h/2,8,2,h-50,2,0,accent,.25,.15,2.0,TRIM);
   }
   for(const y of [edgeInset,h-edgeInset]){
    this.box(w/2,y,-26,w-20,16,74,0,[.11,.16,.19],.28,.86,0,TALL);
   ...
   for(let i=0;i<towerCount;i++){
    ...
    this.box(x,y,-5,tw,td,th,a,[.17,.22,.26],.25,.9,0,TALL);
    this.box(x,y,th*.42,tw*.66,td*.46,10,a,[.42,.48,.52],.28,.84,0,TALL);
   ...
   if(theme===0){
    ...
    for(let i=0;i<4;i++){const side=i<2?-1:1,y=h*(.28+.16*(i%2));this.box(w/2+side*innerR*.63,y,10,18,28,62,0,[.16,.21,.25],.24,.88,0,TALL);this.box(w/2+side*innerR*.63,y,46,5,16,4,0,accent,.24,.16,1.7,TRIM);} 
   }
   // Stronger stage differentiation: each stage gets unmistakable static silhouettes.
   if(stage===0){
    for(let i=0;i<3;i++){const y=h*(.24+.26*i);this.box(w*.16,y,-8,16,26,120,.06,[.13,.20,.22],.24,.90,0,TALL);this.box(w*.84,y,-8,16,26,120,-.06,[.13,.20,.22],.24,.90,0,TALL);this.box(w*.16,y,58,6,18,5,0,accent,.22,.20,2.1,MID);this.box(w*.84,y,58,6,18,5,0,accent,.22,.20,2.1,MID);} 
   }else if(stage===1){
    for(let i=0;i<5;i++){const x=w*(.18+.16*i);this.box(x,h*.18,-4,12,34,92,0,[.18,.14,.22],.24,.88,0,TALL);this.box(x,h*.82,-4,12,34,92,0,[.18,.14,.22],.24,.88,0,TALL);} 
    this.box(w/2,h*.22,24,w*.46,10,10,0,accent,.24,.16,1.4,TRIM);this.box(w/2,h*.78,24,w*.46,10,10,0,accent,.24,.16,1.4,TRIM);
   }else if(stage===2){
    const rr=innerR*.94;for(let i=0;i<8;i++){const a=i*TAU/8,x=w/2+Math.cos(a)*rr,y=h/2+Math.sin(a)*rr;this.box(x,y,18,20,20,72,a,[.22,.18,.13],.23,.84,0,TALL);this.add('orb',x,y,60,4.2,4.2,4.2,0,accent,.16,.08,2.8,MID);} 
   }
   ...
   for(let i=0;i<skyline;i++){
    ...
    this.box(x,y,-18,w0,d0,h0,a,[.12,.16,.20],.26,.86,0,SKY);
    this.box(x,y,h0*.45,w0*.44,d0*.44,10,a,[.43,.46,.49],.26,.78,0,SKY);
  ...
  build(frame){
   ...
   this.dynamicScenery(w,h,stage,seedKey,t);
