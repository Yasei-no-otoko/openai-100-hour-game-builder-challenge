/* NEMESIS PACT — 0.3.4, native WebGL2 + native WebGPU.
...
const WG_FRAME=`struct Frame { arena:vec4f, shock:vec4f, shake:vec4f, screen:vec4f };`;
...
function frameData(f,width,height){return new Float32Array([f.w,f.h,f.t,f.world?Math.min(f.world.stage,2):0,f.shock?.x??.5,f.shock?.y??.5,f.shock?.age||0,f.shock?.strength||0,f.shakeX||0,f.shakeY||0,0,0,width,height,1.1,f.reduced?1:0]);}
class GLBackend{
 ...
 render(groups,f,w,h){const g=this.gl;if(g.isContextLost())return false;this.resize(w,h);const data=frameData(f,w,h);g.bindFramebuffer(g.FRAMEBUFFER,this.full.fb);g.viewport(0,0,w,h);g.enable(g.DEPTH_TEST);g.depthFunc(g.LESS);g.depthMask(true);g.disable(g.BLEND);g.disable(g.CULL_FACE);g.clearColor(.006,.01,.018,0);g.clear(g.COLOR_BUFFER_BIT|g.DEPTH_BUFFER_BIT);g.useProgram(this.program);g.uniform4fv(this.locations.frame,data);this.draws=0;
...
class GPUBackend{
 ...
 async initialize(){const d=this.device;d.pushErrorScope('validation');this.uniform=d.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});this.sampler=d.createSampler({magFilter:'linear',minFilter:'linear',addressModeU:'clamp-to-edge',addressModeV:'clamp-to-edge'});
 ...
 render(groups,f,w,h){this.resize(w,h);const d=this.device;d.queue.writeBuffer(this.uniform,0,frameData(f,w,h));const encoder=d.createCommandEncoder({label:'NEMESIS frame'}),pass=encoder.beginRenderPass({colorAttachments:[{view:this.scene.createView(),clearValue:{r:.006,g:.010,b:.018,a:0},loadOp:'clear',storeOp:'store'}],depthStencilAttachment:{view:this.depth.createView(),depthClearValue:1,depthLoadOp:'clear',depthStoreOp:'discard'}});pass.setPipeline(this.meshPipeline);pass.setBindGroup(0,this.frameGroup);this.draws=0;
...
class Renderer{
 ...
 stats(){return {build:'0.3.4',backend:this.backend,frames:this.active?.frames||0,computeDispatches:this.active?.computeDispatches||0,instances:this.scene.count,draws:this.active?.draws||0,hdr:this.active instanceof GPUBackend||!!this.active?.hdr,errors:[...this.messages]};}
...
return {Renderer,GLBackend,GPUBackend,frameData,GL_VERTEX,GL_FRAGMENT,GL_QUAD,GL_BLUR,GL_COMPOSITE,WG_MESH,WG_BLUR,WG_COMPOSITE,WG_PRESENT};
});
